...
_MF_ZAIM_MAP: dict[tuple[str, str], str] = {
    ...
    # 現金・カード → スキップ（ATM引き出し・カード引き落としは二重計上のため集計対象外）
    ('現金・カード', 'ATM引き出し'):        '',
    ('現金・カード', '電子マネー'):         '',
    ('現金・カード', 'カード引き落とし'):   '',
    ('現金・カード', 'その他現金・カード'): '',
...
def map_to_zaim(mf_large: str, mf_middle: str) -> str:
    """MF (大項目, 中項目) → Zaim カテゴリ名に変換。'' を返した場合はインポートスキップ対象"""
    key = (mf_large, mf_middle)
    if key in _MF_ZAIM_MAP:
        return _MF_ZAIM_MAP[key]   # '' も含めてそのまま返す（スキップ対象）
